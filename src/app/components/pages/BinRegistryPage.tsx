...
export const BinRegistryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("bins onSnapshot error:", err);
        setLoading(false);
        alert("Failed to load bins. Check Firestore rules / indexes.");
      }
    );
  ...
  const handleUpdateBin = async () => {
    ...
    setBusy(true);
    try {
      const ref = doc(db, "bins", editing.id);

      // ✅ Only updating allowed fields (rules enforce too)
      await updateDoc(ref, {
        location: locationValue,
        status: editStatus,
        capacityLiters: cap,
  ...
  const handleDeleteBin = async (row: { id: string } & BinDoc) => {
    ...
    setBusy(true);
    try {
      await deleteDoc(doc(db, "bins", row.id));
      alert("🗑️ Bin deleted.");
    } catch (e: any) {
      console.error(e);
